...
@Component({
  selector: 'app-amber-assets',
  standalone: false, // Assuming this is part of a larger module. If standalone, remove this line.
  templateUrl: './amber-assets.component.html',
  styleUrls: ['./amber-assets.component.scss']
})
export class AmberAssetsComponent implements OnInit {
  // Font Awesome Icons
  ...
  // Data management
  dataSource: Asset[] = []; // Holds all fetched assets for the team (if user is in Amber train)
  ...
  errorMessage: string | null = null; // Specific error message for user feedback

  // Filtering and Sorting properties
  filterText = '';
  sortColumn = '';
  sortDirection: 'asc' | 'desc' = 'asc';
  amberAssets: Asset[] = []; // Holds all assets if the user is in the 'Amber' train
  ...
  loadUserAndTeamAssets(): void {
    ...
    // Get user's team and train first
    if (!currentUserUsername) {
      this.handleError('Please log in to view assets.');
      return;
    }
    ...
    this.userService.getUsers().subscribe({
      next: (users) => {
        ...
        if (currentUser) {
          ...
          // --- CRITICAL: Check if the user belongs to the 'Amber' train ---
          if (this.currentUserAgileTrain === 'Amber') {
            // User is in the Amber train, fetch Amber assets
            this.loadAmberAssets();
            // If we also need to show team-specific assets within Amber, fetch them here.
            // For now, the requirement is to show ALL amber assets if the user belongs to amber.
            // So, the `loadAmberAssets` should be sufficient.
            // If you need team-specific filtering *within* Amber, uncomment and adjust `fetchTeamAssets`:
            /*
            if (this.currentUserScrumTeam) {
               this.fetchTeamAssets(); // Fetch assets for this specific team within Amber
            } else {
               this.handleError('Your Scrum Team is not specified. Cannot load team-specific assets within Amber.');
            }
            */
          } else {
            // User is NOT in the Amber train
            this.errorMessage = 'You do not have permission to view Amber Train assets.';
            this.amberAssets = []; // Ensure no assets are displayed
            this.dataSource = [];
            this.displayedData = [];
            this.isLoadingModules = false;
          }
        } else {
          this.handleError('User details not found. Please check your profile.');
          this.isLoadingModules = false;
        }
      },
      error: (err) => {
  ...
  loadAmberAssets(): void {
    const targetAgileTrain = 'Amber';
    this.assetService.getAssetList(undefined, null, null, targetAgileTrain).subscribe({
      next: (assets: Asset[]) => {
        this.amberAssets = assets; // Store all Amber assets
        this.errorMessage = null; // Clear any previous error

        // Now, update the displayed data with these Amber assets
        // We'll use the first page, default sort, and filter of these assets
        this.dataSource = this.amberAssets; // Set dataSource to all Amber assets
        this.updateTotalPages();
        this.updateDisplayedData(); // Apply initial sort/filter to the Amber assets
      ...
      error: (err) => {
        console.error(`Error fetching assets for Agile Release Train "${targetAgileTrain}":`, err);
        this.handleError(`Could not load assets for Agile Release Train "${targetAgileTrain}".`);
        this.amberAssets = []; // Ensure empty on error
        this.dataSource = [];
        this.displayedData = [];
        this.isLoadingModules = false;
      }
  ...
  // This method would be used if you needed to filter Amber assets by team
  // For the current requirement, we display ALL amber assets.
  fetchTeamAssets(): void {
    ...
    this.assetService.getAssetList(
      ...
      this.currentUserAgileTrain // This should be 'Amber' if this is called
    ).subscribe({
      next: (assets: Asset[]) => {
      ...
      error: (err) => {
        console.error(`Error fetching assets for Scrum Team "${this.currentUserScrumTeam}" in Train "${this.currentUserAgileTrain}":`, err);
  ...
  exportToExcel(): void {
    ...
    try {
      ...
      const dataToExport = [...this.dataSource]; // Use the data that's currently loaded (all amber assets)
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Agile Train Assets'); // Sheet name
      const filename = `AgileTrainAssets_${this.currentUserAgileTrain ? this.currentUserAgileTrain : 'Export'}_${formatDate(new Date(), 'yyyyMMdd_HHmmss', 'en-US')}.xlsx`;
  ...
  private handleError(message: string): void {
    ...
    // Clear assets and reset pagination/sorting if an error occurs or user has no access
    this.amberAssets = [];
    this.dataSource = [];
    this.displayedData = [];
    this.page = 1;
