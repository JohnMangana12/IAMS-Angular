...
import {
  faTools, // For the title icon
  faEdit,
  faTrash,
  faDownload, // For the export button
  faMagnifyingGlass, // For the search input
...
export class SpareItemsComponent implements OnInit {
  ...
  // Ensure these properties exist in your asset objects.
  ...
  getSpareItems(): void {
    ...
    this._assetService.getAssetList().subscribe({
      next: (assets) => {
        // Filter the assets specifically for 'Spare' condition
        this.dataSource = assets.filter(asset =>
          asset.AssetCondition === 'Spare' // Or whatever property correctly identifies a spare item
        );
        console.log('Spare Items data loaded:', this.dataSource);
      ...
      error: (error) => {
        console.error('Error fetching Spare Items:', error);
        this._snackBar.open('Error loading Spare Items. Please try again.', 'Close', { duration: 3000 });
   ...
   // --- Pagination, Filtering, and Sorting Methods ---
    ...
    openEditAssetsForm(asset: any) {
      ...
      modalRef.result.then((result) => {
        if (result === 'success') { // Assuming your modal emits 'success' on save
          ...
          this.getSpareItems(); // Refresh the list
        }
      }, (reason) => {
    ...
    exportToExcel(): void {
      try {
        ...
        XLSX.writeFile(workbook, 'Spare_Assets_Export.xlsx');
