...
import {
  ...
  faSort,     // Import sort icon
  faSortUp,     // Import sort up icon
  faSortDown,   // Import sort down icon
  faMagnifyingGlass // Import magnifying glass icon for search
...
export class AssetListComponent implements OnInit {
  ...
  faDownload = faDownload; // Assign download icon
  faUpload = faUpload;     // Assign upload icon
  faPlus = faPlus;       // Assign plus icon
  faSort = faSort;       // Base sort icon
  faSortUp = faSortUp;     // Sort up icon
  faSortDown = faSortDown;   // Sort down icon
  faMagnifyingGlass = faMagnifyingGlass; // Assign magnifying glass icon
  ...
  displayableColumns: string[] = [
    ...
    'Warranty',
    'PoNumber',
    // You can add others here if you want them to be sortable and visible in headers
  ...
  readonly MIN_LOADING_DURATION_MS = 1500;
  ...
  getAssetList() {
    ...
    this._assetService.getAssetList().pipe(
      map(assets => {
        // Filter out unwanted asset groups
        const filteredAssets = assets.filter(asset =>
          asset.GroupAssetCategory !== 'License Assets' &&
          asset.GroupAssetCategory !== 'Capitalize Assets' &&
          asset.GroupAssetCategory !== '3rd Party Devices' &&
          asset.GroupAssetCategory !== 'Computer Assets'
      ...
      next: (res) => {
        ...
        console.log('Asset list loaded and filtered.', this.dataSource);
      },
      error: (err) => {
        console.error('Error fetching asset list:', err);
        this._snackBar.open('Failed to load asset list. Please try again later.', 'Close', { duration: 5000 });
      }
    });
  }
  ...
  openAddEditAssetsForm(data?: any) {
    ...
    // If 'data' is provided, it's an edit operation. Pass the data to the modal.
    // If 'data' is undefined, it's an add operation.
    if (data) {
    ...
    modalRef.result.then((result) => {
      if (result === 'success') { // Assuming your modal emits 'success' on save
        ...
        this.getAssetList(); // Refresh list
      }
    }, (reason) => {
      // Handle dismissal
  ...
  preprocessAssetData(item: any): any {
      ...
      // Map Excel columns to your asset model properties
  ...
  formatDateForAPI(date: any): string | null {
      ...
      try {
          ...
          // If it's an Excel number, XLSX.read(..., {cellDates: true}) handles it.
          // If it's something else, this might need more robust parsing.
  ...
  importAssets(assets: any[]) {
    ...
    const importObservables = assets.map(asset => {
      ...
      if (!asset.AssetTag || !asset.Description) { // Example validation
        failedCount++;
    ...
    forkJoin(importObservables).pipe(
      finalize(() => {
        ...
        this.getAssetList(); // Refresh the asset list
      ...
      // The next callback here is not strictly needed because finalize handles cleanup
      // and the individual observables in forkJoin already return mapped results.
      // We can log the aggregated results if needed for debugging.
      next: (results) => {
        console.log('All import operations processed.');
      },
      error: (err) => {
        // This error block is for errors in forkJoin itself, which is unlikely if catchError is used.
  ...
  exportToExcel(): void {
    try {
      const dataToExport = [...this.dataSource]; // Use the full dataSource for export
      ...
      // Auto-fit columns
      worksheet['!cols'] = exportConfig.map(col => ({ wch: col.header.length + 5 })); // Initial width based on header length + padding

      const workbook: XLSX.WorkBook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, 'DeltaV Assets');
      XLSX.writeFile(workbook, 'DeltaV_Assets_Export.xlsx');
