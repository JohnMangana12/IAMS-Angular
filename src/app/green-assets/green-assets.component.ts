...
@Component({
 selector: 'app-green-assets',
 standalone: false,
 templateUrl: './green-assets.component.html',
 styleUrls: ['./green-assets.component.scss']
})
export class GreenAssetsComponent implements OnInit {
// Font Awesome Icons
  ...
  greenAssets: Asset[] = []; // Holds all assets if the user is in the 'green' train
    ...
    this.userService.getUsers().subscribe({
      next: (users) => {
        ...
        if (currentUser) {
          ...
          // --- CRITICAL: Check if the user belongs to the 'Green' train ---
          if (this.currentUserAgileTrain === 'Green') {
            // User is in the Amber train, fetch Amber assets
            this.loadGreenAssets();
            ...
            */
            ...
            this.greenAssets = []; // Ensure no assets are displayed
            this.dataSource = [];
            this.displayedData = [];
  ...
  loadGreenAssets(): void {
    const targetAgileTrain = 'Green';
    this.assetService.getAssetList(undefined, null, null, targetAgileTrain).subscribe({
      next: (assets: Asset[]) => {
        this.greenAssets = assets; // Store all Amber assets
        ...
        this.dataSource = this.greenAssets; // Set dataSource to all Amber assets
      ...
      error: (err) => {
        ...
        this.greenAssets = []; // Ensure empty on error
        this.dataSource = [];
        this.displayedData = [];
        this.isLoadingModules = false;
      }
